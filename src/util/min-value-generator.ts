...
/**
 * Returns an iterator that points to the next min (according to the provided comparator)
 ...
 */
export function * minValueGenerator<S, T>(iterable: Iterable<S>, mapper: (value: S) => T, comparator: (first: T, second: T) => number) {
    let start = 0;
